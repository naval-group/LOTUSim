...
def render_xacro(context: LaunchContext, agent_config, ns_base):
    ...
    ns_base_str = context.perform_substitution(ns_base)
    ...
    for agent_type in agent_config_json_data.keys():
        ...
        sdf_file  =  os.path.join('models', agent_type, 'model.sdf')
        for agent_data in agent_config_json_data[agent_type]:
            agent_data.update(
                {'sdf_file': sdf_file, 
                 'ns': ns_base_str + str("{:03d}".format(ns_index))
                }
            )
            ...
            # agent_fullnames.append(ns_base_str + str(ns_index) + agent_data['name'])
            ns_index = ns_index + 1

def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument('agent_config', default_value='default_agent_config.json'),
        DeclareLaunchArgument('ns_base', default_value=''),
