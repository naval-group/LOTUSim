...
public:
    ...
    RadarTestSpawner() : Node("radar_test_spawner")
    {
        pose_subscription_ = this->create_subscription<VesselPositionArray>(
            "/lotusim/poses",
            rclcpp::QoS(10),
            std::bind(&RadarTestSpawner::poses_callback, this, std::placeholders::_1));

        mas_action_client_ =
            rclcpp_action::create_client<MASCmd>(this, "/lotusim/mas_cmd");

        mas_array_action_client_ =
            rclcpp_action::create_client<MASCmdArray>(this, "/lotusim/mas_cmd_array");
    }
    ...
    void spawn_static_commando()
    {
        ...
        opts.result_callback = [this](const GoalHandleMASCmd::WrappedResult & result) {
            RCLCPP_INFO(this->get_logger(), "Commando ship spawned: %s",
    ...
    void spawn_front_target()
    {
        ...
        auto opts = rclcpp_action::Client<MASCmd>::SendGoalOptions();
        opts.result_callback = [this](const GoalHandleMASCmd::WrappedResult & result) {
            RCLCPP_INFO(this->get_logger(), "LRAUV target spawned: %s",
                result.result->name.c_str());
        };
        mas_action_client_->async_send_goal(goal_msg, opts);
    }
    ...
    void spawn_lrauv_circle(int n_ships, double radius_m)
    {
        if (!mas_array_action_client_->wait_for_action_server(5s)) {
            RCLCPP_ERROR(this->get_logger(), "MASCmdArray action server not available");
            return;
        }
        ...
        for (int i = 0; i < n_ships; ++i) {
            ...
            double lon_offset = (radius_m / (111111.0 * std::cos(SPAWN_LATITUDE * M_PI / 180.0)))
                                * std::sin(angle);
        ...
        opts.result_callback =
            [this, n_ships](const GoalHandleMASCmdArray::WrappedResult & result) {
                RCLCPP_INFO(this->get_logger(),
                    "%d LRAUV ships spawned around commando (status %d)",
                    n_ships, static_cast<int>(result.code));
            };
    ...
    void poses_callback(const VesselPositionArray::SharedPtr msg)
    {
        for (const auto & vessel : msg->vessels) {
            vessel_poses_[vessel.vessel_name] =
                std::make_pair(vessel.geo_point.latitude, vessel.geo_point.longitude);
        }
    }
