...
GLuint EngineToQtInterface::TextureId(gz::rendering::CameraPtr &_camera)
{
    if (!this->NeedsFallback(_camera)) {
        ...
        // We don't care about buffering / performance so we perform a
        // synchronous glTexSubImage2D copy. This is a slow fallback path. We
        // don't want to make the code unnecessary complex.
        glFuncs->glBindTexture(GL_TEXTURE_2D, this->dataPtr->fallbackTexture);
        ...
        glFuncs->glTexSubImage2D(
            ...
            this->dataPtr->fallbackImage->Data());
