...
TEST(BridgeConfig, Minimum)
{
...

    {
        ...
        EXPECT_EQ(
            ros_gz_bridge::kDefaultPublisherQueue, config.publisher_queue_size);
        EXPECT_EQ(
            ros_gz_bridge::kDefaultSubscriberQueue,
...
{
    auto results =
        ros_gz_bridge::readFromYamlFile("test/config/minimum_ign.yaml");
    EXPECT_EQ(4u, results.size());
...
TEST(BridgeConfig, ParseDirection)
{
  ...
  - topic_name: foo
    ...
    direction: ROS_TO_GZ
        ...
        auto results = ros_gz_bridge::readFromYamlString(yaml);
        EXPECT_EQ(
            ros_gz_bridge::BridgeDirection::ROS_TO_GZ, results[0].direction);
    }
  ...
  - topic_name: foo
    ...
    direction: GZ_TO_ROS
        ...
        auto results = ros_gz_bridge::readFromYamlString(yaml);
        EXPECT_EQ(
            ros_gz_bridge::BridgeDirection::GZ_TO_ROS, results[0].direction);
    }
