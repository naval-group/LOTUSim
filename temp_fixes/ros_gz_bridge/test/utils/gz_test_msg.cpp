...
void createTestMsg(ignition::msgs::Boolean &_msg) { _msg.set_data(true); }
...
void createTestMsg(ignition::msgs::Float &_msg) { _msg.set_data(1.5); }
...
void createTestMsg(ignition::msgs::Double &_msg) { _msg.set_data(1.5); }
...
void createTestMsg(ignition::msgs::Int32 &_msg) { _msg.set_data(-10); }
...
void createTestMsg(ignition::msgs::UInt32 &_msg) { _msg.set_data(1); }
...
void createTestMsg(ignition::msgs::StringMsg &_msg) { _msg.set_data("string"); }
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Param_V> &_msg)
{
    ...
    compareTestMsg(std::make_shared<ignition::msgs::Header>(_msg->header()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Param>(_msg->param().Get(0)));
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Pose> &_msg)
{
    if (_msg->header().data_size() > 0) {
        compareTestMsg(
            std::make_shared<ignition::msgs::Header>(_msg->header()));
    ...
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->position()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Quaternion>(_msg->orientation()));
}
...
void compareTestMsg(
    const std::shared_ptr<ignition::msgs::PoseWithCovariance> &_msg)
{
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->pose().position()));
    compareTestMsg(std::make_shared<ignition::msgs::Quaternion>(
        _msg->pose().orientation()));
    for (int i = 0; i < 36; i++) {
...
void compareTestMsg(
    const std::shared_ptr<ignition::msgs::TwistWithCovariance> &_msg)
{
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->twist().linear()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->twist().angular()));
    for (int i = 0; i < 36; i++) {
        EXPECT_EQ(_msg->covariance().data()[i], i);
    }
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::JointWrench> &_msg)
{
    ...
    compareTestMsg(
        std::make_shared<ignition::msgs::Wrench>(_msg->body_1_wrench()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Wrench>(_msg->body_2_wrench()));
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Contact> &_msg)
{
    ...
    createTestMsg(expected_msg);
    compareTestMsg(
        std::make_shared<ignition::msgs::Entity>(_msg->collision1()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Entity>(_msg->collision2()));
    ...
    for (int i = 0; i < expected_msg.depth_size(); i++) {
        ...
        compareTestMsg(
            std::make_shared<ignition::msgs::Vector3d>(_msg->position(i)));
        compareTestMsg(
            std::make_shared<ignition::msgs::Vector3d>(_msg->normal(i)));
        compareTestMsg(
            std::make_shared<ignition::msgs::JointWrench>(_msg->wrench(i)));
    }
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Contacts> &_msg)
{
    ...
    for (int i = 0; i < expected_msg.contact_size(); i++) {
        compareTestMsg(
            std::make_shared<ignition::msgs::Contact>(_msg->contact(i)));
    }
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::CameraInfo> &_msg)
{
    ...
    ASSERT_EQ(
        expected_msg.rectification_matrix_size(),
        _msg->rectification_matrix_size());
    for (auto i = 0; i < expected_msg.rectification_matrix_size(); ++i) {
        EXPECT_EQ(
            expected_msg.rectification_matrix(i),
            _msg->rectification_matrix(i));
    }
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::IMU> &_msg)
{
    ...
    compareTestMsg(
        std::make_shared<ignition::msgs::Quaternion>(_msg->orientation()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->angular_velocity()));
    compareTestMsg(std::make_shared<ignition::msgs::Vector3d>(
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Model> &_msg)
{
    ...
    for (auto i = 0; i < _msg->joint_size(); ++i) {
        ...
        compareTestMsg(
            std::make_shared<ignition::msgs::Axis>(_msg->joint(i).axis1()));
    }
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::LaserScan> &_msg)
{
    ...
    EXPECT_DOUBLE_EQ(
        expected_msg.vertical_angle_min(), _msg->vertical_angle_min());
    EXPECT_DOUBLE_EQ(
        expected_msg.vertical_angle_max(), _msg->vertical_angle_max());
    EXPECT_DOUBLE_EQ(
        expected_msg.vertical_angle_step(), _msg->vertical_angle_step());
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Magnetometer> &_msg)
{
    compareTestMsg(std::make_shared<ignition::msgs::Header>(_msg->header()));
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->field_tesla()));
}
...
void compareTestMsg(
    const std::shared_ptr<ignition::msgs::OdometryWithCovariance> &_msg)
{
    compareTestMsg(std::make_shared<ignition::msgs::Header>(_msg->header()));
    compareTestMsg(std::make_shared<ignition::msgs::PoseWithCovariance>(
        _msg->pose_with_covariance()));
    compareTestMsg(std::make_shared<ignition::msgs::TwistWithCovariance>(
        _msg->twist_with_covariance()));
}
    ...
    for (uint32_t j = 0; j < height; ++j) {
        for (uint32_t i = 0; i < width; ++i) {
            *reinterpret_cast<float *>(
                msgBufferIndex + _msg.field(0).offset()) = j * width + i;
...
void compareTestMsg(
    const std::shared_ptr<ignition::msgs::PointCloudPacked> &_msg)
{
    compareTestMsg(std::make_shared<ignition::msgs::Header>(_msg->header()));
...
void compareTestMsg(
    const std::shared_ptr<ignition::msgs::JointTrajectoryPoint> &_msg)
{
    ...
    EXPECT_EQ(
        expected_msg.time_from_start().sec(), _msg->time_from_start().sec());
    EXPECT_EQ(
        expected_msg.time_from_start().nsec(), _msg->time_from_start().nsec());
}
...
void compareTestMsg(
    const std::shared_ptr<ignition::msgs::JointTrajectory> &_msg)
{
    ...
    for (int i = 0; i < _msg->points_size(); ++i) {
        compareTestMsg(std::make_shared<ignition::msgs::JointTrajectoryPoint>(
            _msg->points(i)));
    }
}
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::Light> &_msg)
{
    ...
    compareTestMsg(
        std::make_shared<ignition::msgs::Vector3d>(_msg->direction()));

    EXPECT_EQ(expected_msg.name(), _msg->name());
    EXPECT_EQ(expected_msg.type(), _msg->type());

    EXPECT_FLOAT_EQ(
        expected_msg.attenuation_constant(), _msg->attenuation_constant());
    EXPECT_FLOAT_EQ(
        expected_msg.attenuation_linear(), _msg->attenuation_linear());
    EXPECT_FLOAT_EQ(
        expected_msg.attenuation_quadratic(), _msg->attenuation_quadratic());
...
void compareTestMsg(const std::shared_ptr<ignition::msgs::GUICamera> &_msg)
{
    ...
    compareTestMsg(std::make_shared<ignition::msgs::Pose>(_msg->pose()));
    compareTestMsg(
        std::make_shared<ignition::msgs::TrackVisual>(_msg->track()));
