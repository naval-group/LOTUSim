...
void createTestMsg(gz::msgs::Boolean &_msg) { _msg.set_data(true); }
...
void createTestMsg(gz::msgs::Float &_msg) { _msg.set_data(1.5); }
...
void createTestMsg(gz::msgs::Double &_msg) { _msg.set_data(1.5); }
...
void createTestMsg(gz::msgs::Int32 &_msg) { _msg.set_data(-10); }
...
void createTestMsg(gz::msgs::UInt32 &_msg) { _msg.set_data(1); }
...
void createTestMsg(gz::msgs::StringMsg &_msg) { _msg.set_data("string"); }
...
{
    compareTestMsg(
        std::make_shared<gz::msgs::Vector3d>(_msg->pose().position()));
    compareTestMsg(
        std::make_shared<gz::msgs::Quaternion>(_msg->pose().orientation()));
    for (int i = 0; i < 36; i++) {
...
{
    compareTestMsg(
        std::make_shared<gz::msgs::Vector3d>(_msg->twist().linear()));
    compareTestMsg(
        std::make_shared<gz::msgs::Vector3d>(_msg->twist().angular()));
    for (int i = 0; i < 36; i++) {
        EXPECT_EQ(_msg->covariance().data()[i], i);
    }
}
...
void compareTestMsg(const std::shared_ptr<gz::msgs::Contact> &_msg)
{
    ...
    for (int i = 0; i < expected_msg.depth_size(); i++) {
        ...
        compareTestMsg(
            std::make_shared<gz::msgs::JointWrench>(_msg->wrench(i)));
    }
}
...
void compareTestMsg(const std::shared_ptr<gz::msgs::IMU> &_msg)
{
    ...
    compareTestMsg(std::make_shared<gz::msgs::Quaternion>(_msg->orientation()));
    compareTestMsg(
        std::make_shared<gz::msgs::Vector3d>(_msg->angular_velocity()));
    compareTestMsg(
        std::make_shared<gz::msgs::Vector3d>(_msg->linear_acceleration()));
}
...
void compareTestMsg(const std::shared_ptr<gz::msgs::Model> &_msg)
{
    ...
    for (auto i = 0; i < _msg->joint_size(); ++i) {
        ...
        compareTestMsg(
            std::make_shared<gz::msgs::Axis>(_msg->joint(i).axis1()));
    }
}
...
void compareTestMsg(
    const std::shared_ptr<gz::msgs::OdometryWithCovariance> &_msg)
{
    compareTestMsg(std::make_shared<gz::msgs::Header>(_msg->header()));
    compareTestMsg(std::make_shared<gz::msgs::PoseWithCovariance>(
        _msg->pose_with_covariance()));
    compareTestMsg(std::make_shared<gz::msgs::TwistWithCovariance>(
...
void compareTestMsg(const std::shared_ptr<gz::msgs::JointTrajectory> &_msg)
{
    ...
    for (int i = 0; i < _msg->points_size(); ++i) {
        compareTestMsg(
            std::make_shared<gz::msgs::JointTrajectoryPoint>(_msg->points(i)));
    }
}
