...
void compareTestMsg(const std::shared_ptr<gz::msgs::JointTrajectoryPoint> &_msg)
{
    ...
    EXPECT_EQ(
        expected_msg.time_from_start().sec(),
        _msg->time_from_start().sec());
    EXPECT_EQ(
        expected_msg.time_from_start().nsec(),
        _msg->time_from_start().nsec());
}
...
void compareTestMsg(const std::shared_ptr<gz::msgs::Light> &_msg)
{
    ...
    EXPECT_FLOAT_EQ(
        expected_msg.attenuation_linear(),
        _msg->attenuation_linear());
    EXPECT_FLOAT_EQ(
    ...
    const std::shared_ptr<gz::msgs::AnnotatedAxisAligned2DBox_V> &_msg)
{
    ...
    for (size_t i = 0; i < 4; i++) {
        compareTestMsg(std::make_shared<gz::msgs::AnnotatedAxisAligned2DBox>(
            _msg->annotated_box(i)));
    }
}
    ...
    const std::shared_ptr<gz::msgs::AnnotatedOriented3DBox_V> &_msg)
{
    ...
    for (size_t i = 0; i < 4; i++) {
        compareTestMsg(std::make_shared<gz::msgs::AnnotatedOriented3DBox>(
            _msg->annotated_box(i)));
    }
}
