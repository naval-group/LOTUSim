...
{
    std::cout
        << "Bridge a collection of ROS2 and Gazebo Transport topics and "
           "services.\n\n"
        << "  parameter_bridge [<topic@ROS2_type@Ign_type> ..] "
        << " [<service@ROS2_srv_type[@Ign_req_type@Ign_rep_type]> ..]\n\n"
        << "Topics: The first @ symbol delimits the topic name from the "
           "message types.\n"
        ...
        << "type.\n\n"
        << "Services: The first @ symbol delimits the service name from the "
           "types.\n"
        << "Following the first @ symbol is the ROS service type.\n"
        << "Optionally, you can include the Gazebo request and response type\n"
        << "separated by the @ symbol.\n"
        << "It is only supported to expose Gazebo servces as ROS services, "
           "i.e.\n"
           "the ROS service will forward request to the Gazebo service and "
           "then forward\n"
