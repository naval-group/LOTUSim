...
std::optional<BridgeConfig> parseEntry(const YAML::Node& yaml_node)
{
    ...
    if (!yaml_node.IsMap()) {
        RCLCPP_ERROR(logger, "Could not parse entry: entry must be a YAML map");
        return {};
    }
    ...
    if (yaml_node[kTopicName] && yaml_node[kRosTopicName]) {
        RCLCPP_ERROR(
            logger,
            "Could not parse entry: %s and %s are mutually exclusive",
            kTopicName,
            kRosTopicName);
        return {};
    }
    ...
    if (!yaml_node[kRosTypeName] || gz_type_name.empty()) {
        RCLCPP_ERROR(
            logger,
            "Could not parse entry: both %s and %s must be set",
            kRosTypeName,
            kGzTypeName);
        return {};
    }
    ...
    if (yaml_node[kDirection]) {
        ...
        if (dirStr == kBidirectional) {
            ...
            RCLCPP_WARN(
                logger,
                "%s constant is deprecated, migrate to %s",
                kIgnToRos,
                kGzToRos);
            ...
            RCLCPP_ERROR(
                logger,
                "Could not parse entry: invalid direction [%s]",
                dirStr.c_str());
            return {};
        }
    }
