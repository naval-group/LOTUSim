...
bool send_response_on_error(RosResT &ros_response);
...
public:
    ...
    rclcpp::ServiceBase::SharedPtr create_ros_service(
        ...
        const std::string &service_name) override
    {
        return ros_node->create_service<RosServiceT>(
            ...
            [gz_node = std::move(gz_node), service_name](
                ...
                callback = [srv_handle = std::move(srv_handle),
                            reqid](const IgnReplyT &reply, const bool result) {
                ...
                gz_node->Request(service_name, gz_req, callback);
            });
    }
