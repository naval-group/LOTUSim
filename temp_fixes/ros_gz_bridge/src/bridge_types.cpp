...
int main(int argc, char *argv[])
{
    ...
    if (find_command_option(args, "-h") ||
        find_command_option(args, "--help")) {
        std::stringstream ss;
        ss << "Usage:" << std::endl;
        ss << " -h, --help: This message." << std::endl;
        ss << " --print-pairs: Print a list of the supported ROS 2 <=> GZ "
              "conversion pairs.";
        ss << std::endl;
        ...
        ss << std::endl;
        ss << " --get-gz-to-ros <MSG>: Get the ROS 2 mapping for an Gazebo "
              "Type.";
        ss << std::endl;
        ss << " --get-ros-to-gz <MSG>: Get the Gazebo mapping for a ROS 2 "
              "Type.";
        std::cout << ss.str();
        return 0;
    }
    ...
    if (get_flag_option(args, "--get-ros-to-gz")) {
        ...
        auto found =
            ros_gz_bridge::get_ros_to_gz_mapping(args[2], gz_type_name);
        if (found) {
            ...
            std::cout
                << "Type [" << args[2]
                << "] has no mapping, use --print-ros to see supported types"
                << std::endl;
            return 1;
        }
    }
        ...
        auto found =
            ros_gz_bridge::get_gz_to_ros_mapping(args[2], ros_type_name);
        if (found) {
            ...
            std::cout
                << "Type [" << args[2]
                << "] has no mapping, use --print-gz to see supported types"
                << std::endl;
            return 1;
        }
    }
        ...
        if (mappings.size() > 0) {
            ...
            std::cout << "No message type conversion pairs supported."
                      << std::endl;
        }
    }
        ...
        if (mappings.size() > 0) {
            std::cout
                << "Supported ROS <=> Gazebo message type conversion pairs:"
                << std::endl;
            for (auto &pair : mappings) {
                std::cout << "  - '" << pair.first << "' (ROS) <=> '"
                          << pair.second << "' (GZ)" << std::endl;
            }
        }
        else {
            std::cout << "No message type conversion pairs supported."
