    ...
    vision_msgs::msg::Detection2D& ros_msg)
{
    ...
    ros_msg.bbox.center.position.x =
        (gz_msg.box().min_corner().x() + gz_msg.box().max_corner().x()) / 2;
    ros_msg.bbox.center.position.y =
        (gz_msg.box().min_corner().y() + gz_msg.box().max_corner().y()) / 2;
    ros_msg.bbox.size_x =
        gz_msg.box().max_corner().x() - gz_msg.box().min_corner().x();
    ros_msg.bbox.size_y =
        gz_msg.box().max_corner().y() - gz_msg.box().min_corner().y();
}
    ...
    gz::msgs::AnnotatedAxisAligned2DBox_V& gz_msg)
{
    ...
    for (const auto& ros_box : ros_msg.detections) {
        gz::msgs::AnnotatedAxisAligned2DBox* gz_box =
            gz_msg.add_annotated_box();
