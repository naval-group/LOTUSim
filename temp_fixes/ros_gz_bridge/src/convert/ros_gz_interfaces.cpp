    ...
    ros_gz_interfaces::msg::Entity &ros_msg)
{
    ...
    if (gz_msg.type() == gz::msgs::Entity::Type::Entity_Type_NONE) {
        ...
        std::cerr << "Unsupported Entity [" << gz_msg.type() << "]"
                  << std::endl;
    }
}
    ...
    gz::msgs::Light &gz_msg)
{
    ...
    if (ros_msg.type == 0) {
        ...
        gz_msg.set_type(gz::msgs::Light_LightType::Light_LightType_DIRECTIONAL);
    }
    ...
    ros_gz_interfaces::msg::Light &ros_msg)
{
    ...
    ros_msg.name = gz_msg.name();
    if (gz_msg.type() == gz::msgs::Light_LightType::Light_LightType_POINT) {
        ros_msg.type = 0;
    } else if (
        gz_msg.type() == gz::msgs::Light_LightType::Light_LightType_SPOT) {
        ros_msg.type = 1;
    } else if (
        gz_msg.type() ==
    ...
    switch (ros_msg.entity_match) {
        case ros_gz_interfaces::msg::MaterialColor::FIRST:
            gz_msg.set_entity_match(
                EntityMatch::MaterialColor_EntityMatch_FIRST);
            break;
    ...
    ros_gz_interfaces::msg::MaterialColor &ros_msg)
{
    ...
    if (gz_msg.entity_match() == EntityMatch::MaterialColor_EntityMatch_FIRST) {
        ...
        gz_msg.entity_match() == EntityMatch::MaterialColor_EntityMatch_ALL) {
        /* *INDENT-ON* */
        ros_msg.entity_match = ros_gz_interfaces::msg::MaterialColor::ALL;
    } else {
        std::cerr << "Unsupported EntityMatch [" << gz_msg.entity_match() << "]"
                  << std::endl;
    }
    convert_gz_to_ros(gz_msg.header(), ros_msg.header);
    ...
    ros_gz_interfaces::msg::SensorNoise &ros_msg)
{
    ...
    if (gz_msg.type() == gz::msgs::SensorNoise_Type::SensorNoise_Type_NONE) {
        ...
        gz_msg.type() ==
        gz::msgs::SensorNoise_Type::SensorNoise_Type_GAUSSIAN) {
        ros_msg.type = 2;
    } else if (
        gz_msg.type() ==  // NOLINT
        gz::msgs::SensorNoise_Type::
            SensorNoise_Type_GAUSSIAN_QUANTIZED)  // NOLINT
    {                                             // NOLINT
        ros_msg.type = 3;
    }
    ...
    ros_gz_interfaces::msg::ParamVec &ros_msg)
{
    ...
    for (int childIdx = 0; childIdx < gz_msg.children().size(); ++childIdx) {
        ...
        for (size_t entryIdx = 0; entryIdx < child_vec.params.size();
             ++entryIdx) {
            auto ros_param = child_vec.params[entryIdx];
            ros_param.name =
                "child_" + std::to_string(childIdx) + "/" + ros_param.name;
    ...
    ros_gz_interfaces::msg::ParamVec &ros_msg)
{
    ...
    for (int paramIdx = 0; paramIdx < gz_msg.param().size(); ++paramIdx) {
        ...
        for (size_t entryIdx = 0; entryIdx < param_vec.params.size();
             ++entryIdx) {
            auto ros_param = param_vec.params[entryIdx];
            ros_param.name =
                "param_" + std::to_string(paramIdx) + "/" + ros_param.name;
            ros_msg.params.push_back(ros_param);
        }
    }
}
    ...
    gz::msgs::WorldControl &gz_msg)
{
    ...
    convert_ros_to_gz(
        ros_msg.run_to_sim_time,
        *gz_msg.mutable_run_to_sim_time());
}
