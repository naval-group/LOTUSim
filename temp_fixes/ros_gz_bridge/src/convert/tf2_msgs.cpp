...
void convert_ros_to_gz(
    const tf2_msgs::msg::TFMessage &ros_msg, ignition::msgs::Pose_V &gz_msg)
{
    gz_msg.clear_pose();
    ...
    if (!ros_msg.transforms.empty()) {
        convert_ros_to_gz(
            ros_msg.transforms[0].header, (*gz_msg.mutable_header()));
    }
}

template <>
void convert_gz_to_ros(
    const ignition::msgs::Pose_V &gz_msg, tf2_msgs::msg::TFMessage &ros_msg)
{
