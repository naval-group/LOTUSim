...
// to verify behaviors that couldn't easily be captured by the generic test
// framework
...
TEST_F(RosToGzTest, FlatParameters)
{
    ...
    EXPECT_EQ(
        Any_ValueType::Any_ValueType_NONE,
        gz_msg.params().at(kParamNotSetName).type());

    ASSERT_NE(gz_msg.params().end(), gz_msg.params().find(kParamBoolName));
    EXPECT_EQ(
        Any_ValueType::Any_ValueType_BOOLEAN,
        gz_msg.params().at(kParamBoolName).type());
    ...
    EXPECT_EQ(
        Any_ValueType::Any_ValueType_INT32,
        gz_msg.params().at(kParamIntName).type());
    ...
    EXPECT_EQ(
        Any_ValueType::Any_ValueType_DOUBLE,
        gz_msg.params().at(kParamDoubleName).type());
    EXPECT_EQ(
        kParamDoubleValue, gz_msg.params().at(kParamDoubleName).double_value());

    ASSERT_NE(gz_msg.params().end(), gz_msg.params().find(kParamStringName));
    EXPECT_EQ(
        Any_ValueType::Any_ValueType_STRING,
        gz_msg.params().at(kParamStringName).type());
    EXPECT_EQ(
        kParamStringValue, gz_msg.params().at(kParamStringName).string_value());
}
