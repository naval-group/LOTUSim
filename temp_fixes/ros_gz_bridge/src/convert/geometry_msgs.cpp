    ...
    gz::msgs::PoseWithCovariance& gz_msg)
{
    convert_ros_to_gz(
        ...
        *gz_msg.mutable_pose()->mutable_position());
    convert_ros_to_gz(
...
{
    convert_ros_to_gz(
        ros_msg.twist.linear,
        (*gz_msg.mutable_twist()->mutable_linear()));
    convert_ros_to_gz(
        ros_msg.twist.angular,
        (*gz_msg.mutable_twist()->mutable_angular()));
    for (const auto& elem : ros_msg.covariance) {
        gz_msg.mutable_covariance()->add_data(elem);
    }
}
    ...
    gz::msgs::TwistWithCovariance& gz_msg)
{
    convert_ros_to_gz(
        ros_msg.header,
        (*gz_msg.mutable_twist()->mutable_header()));
    convert_ros_to_gz(ros_msg.twist, gz_msg);
}
