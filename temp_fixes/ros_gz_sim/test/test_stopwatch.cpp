...
void runTimer(ros_gz_sim::Stopwatch& _time)
{
    // Start the timer
    ...
    EXPECT_EQ(rclcpp::Duration(0, 0U), _time.ElapsedStopTime());

    // Wait for some time...
    std::this_thread::sleep_for(1000ms);
    // Now the elapsed time should be greater than or equal to the time slept.
    EXPECT_GE(_time.ElapsedRunTime(), rclcpp::Duration(1, 0U));
    ...
    // slept.
    EXPECT_GE(_time.ElapsedStopTime(), rclcpp::Duration(1, 0U));
    ...
    // two sleep times.
    EXPECT_GE(_time.ElapsedRunTime(), rclcpp::Duration(2, 0U));
}
...
TEST_F(Stopwatch, SimTime)
{
    ...
    // Now the elapsed time should be greater than or equal to the time slept.
    EXPECT_GE(watch.ElapsedRunTime(), rclcpp::Duration(1, 0U));
    ...
    // The elapsed stop time should be greater than or equal to the time
    // moved.
    EXPECT_GE(watch.ElapsedStopTime(), rclcpp::Duration(1, 0U));
    ...
    // The elapsed time should be greater than or equal to the the previous
    // two sleep times.
    EXPECT_GE(watch.ElapsedRunTime(), rclcpp::Duration(2, 0U));
}
...
TEST_F(Stopwatch, Constructor)
{
    ...
    ros_gz_sim::Stopwatch watch2(watch);
    EXPECT_EQ(watch, watch2);

    ros_gz_sim::Stopwatch watch3(std::move(watch2));
...
TEST_F(Stopwatch, EqualOperator)
{
    ...
    ros_gz_sim::Stopwatch watch;
    watch.SetClock(clock);
    ros_gz_sim::Stopwatch watch2;
    watch2.SetClock(clock);
    ros_gz_sim::Stopwatch watch3;
