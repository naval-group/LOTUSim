...
int main(int _argc, char** _argv)
{
    ...
    if (world_name.empty()) {
        // If caller doesn't provide a world name, get list of worlds from
        // gz-sim server
    ...
    if (!FLAGS_file.empty()) {
        ...
        if (ros2_node->get_parameter(FLAGS_param, xmlStr)) {
            ...
            RCLCPP_ERROR(
                ros2_node->get_logger(),
                "Failed to get XML from param [%s].",
                FLAGS_param.c_str());
            return -1;
        }
        ...
        description_subs =
            ros2_node->create_subscription<std_msgs::msg::String>(
                FLAGS_topic,
        ...
        do {
            RCLCPP_INFO(
                ros2_node->get_logger(),
                "Waiting messages on topic [%s].",
                FLAGS_topic.c_str());
            future_ret =
                executor.spin_until_future_complete(xml_future, timeout);
        } while (rclcpp::ok() &&
                 future_ret != rclcpp::FutureReturnCode::SUCCESS);
        ...
        if (future_ret == rclcpp::FutureReturnCode::SUCCESS) {
            ...
            RCLCPP_ERROR(
                ros2_node->get_logger(),
                "Failed to get XML from topic [%s].",
                FLAGS_topic.c_str());
            return -1;
        }
    } else {
        RCLCPP_ERROR(
            ros2_node->get_logger(),
            "Must specify either -file, -param, -stdin or -topic");
        return -1;
    }
    ...
    if (executed) {
        ...
        RCLCPP_ERROR(
            ros2_node->get_logger(),
            "Request to create entity from service [%s] timed out..\n %s",
            service.c_str(),
            req.DebugString().c_str());
    }
