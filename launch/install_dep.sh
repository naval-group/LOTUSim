...
echo -e "\n🔧 Starting install of ROS and Gazebo..." >&2

apt-get update -qq >/dev/null 2>&1
apt-get install -y -qq software-properties-common >/dev/null 2>&1
add-apt-repository -y universe >/dev/null 2>&1
apt-get -y -qq install locales tzdata lsb-release curl gnupg2 ca-certificates >/dev/null 2>&1
...
if [ ! -f /usr/share/keyrings/gazebo-archive-keyring.gpg ]; then
  curl -fsSL https://packages.osrfoundation.org/gazebo.key 2>/dev/null | gpg --dearmor -o /usr/share/keyrings/gazebo-archive-keyring.gpg 2>/dev/null
fi
...
if ! grep -Fxq "$GAZEBO_SOURCE_LINE" "$GAZEBO_LIST_FILE" 2>/dev/null; then
  echo "$GAZEBO_SOURCE_LINE" | tee "$GAZEBO_LIST_FILE" >/dev/null 2>&1
fi
...
update-alternatives --install /usr/bin/c++ c++ /usr/bin/clang++ 100 >/dev/null 2>&1
