...
std::string LOTUSIM_PATH = LOTUSIM_WS + "install/lotusim";
...
void build()
{
    system("source /opt/ros/humble/setup.bash");
    chdir(LOTUSIM_WS.c_str());

    std::string cmd = "colcon build --merge-install";
    if (DEBUG) {
        std::cout
...
void run_simulation(const std::string& world_name)
{
    ...
    system("source /opt/ros/humble/setup.bash");
    system(("source " + LOTUSIM_WS + "/install/setup.bash").c_str());
    ...
    std::string command = "gz sim " + std::string(DEBUG ? "-v4 " : "") +
                          (GUI ? "-s " : "") + "-r " + world_file;
