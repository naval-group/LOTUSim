    ...
    : Agent("agent_entity", options)
{
    ...
    // Publisher to Gazebo, for demo purposes only
    ...
    // Timer example that uses the node's clock
    ...
    if (configure_on_startup) {
        ...
        transition_.id = 1; // Id for to go from unconfigured to configured
        this->self_change_state(transition_);
    }
}
...
/// @brief Assigns spawn and despawn behavior based on values of arguments.
/// Either pass the sdf_filename_ or the sdf_file_.
/// @param pose_str_ A pose string written as "posx poy poz rotx roty rotz"
/// @param sdf_filename_ Name of the .sdf model in assets/models as
/// "<model_name>"
/// @param sdf_file_ The whole sdf file. Usually, it has been generated by xacro
...
/// @brief Implement your own logic here
void AgentEntity::timer_callback()
{
    ...
    if (!pose_pub_->is_activated()) {
        ...
        RCLCPP_DEBUG(
            get_logger(),
            "Lifecycle publisher is active. Publishing...");
