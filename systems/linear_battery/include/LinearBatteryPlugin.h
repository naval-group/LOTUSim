...
public:
    void OnEnableRecharge(const gz::msgs::Boolean &_req);

    /// \brief Callback executed to stop recharging.
    /// \param[in] _req This value should be true.
public:
    void OnDisableRecharge(const gz::msgs::Boolean &_req);
...
public:
    ...
    /// \brief Name of model, only used for printing warning when battery
    /// drains.
public:
...
public:
    gz::sim::Entity batteryEntity{gz::sim::v8::kNullEntity};
...
public:
    std::atomic_bool startCharging{false};
...
public:
    ...
    /// \brief Book keep the last time printed, so as to not pollute dbg
    /// messages in minutes
public:
    int lastPrintTime = -1;
...
public:
    gz::sim::Model model{gz::sim::v8::kNullEntity};
...
class LinearBatteryPlugin : public gz::sim::System,
...
private:
    std::unique_ptr<LinearBatteryPluginPrivate> dataPtr;
};
