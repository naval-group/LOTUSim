...
bool RadarSensor::CustomSensorLoad(const sdf::Sensor& _sdf)
{
    ...
    m_radar_scan_pub = m_ros_node->create_publisher<radar_msgs::msg::RadarScan>(
    ...
    const gz::sim::EntityComponentManager& _ecm)
{
    ...
    for (auto& p : points) {
        radar_msgs::msg::RadarReturn det;
        det.range = std::sqrt(p[0] * p[0] + p[1] * p[1]);
        det.azimuth = std::atan2(p[1], p[0]);
        det.elevation = 0;
        det.doppler_velocity = -1;
        det.amplitude = 1.0f;  // Hardcoded for now
        scan_msg.returns.push_back(det);
    }
