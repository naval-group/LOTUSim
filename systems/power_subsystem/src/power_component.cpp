 ...
 */
 ...
 namespace lotusim::power_subsystem{
        ...
        component_name_(component_name)
    {
        ...
        RCLCPP_INFO( 
            node_->get_logger(),
            "[PowerComponent] '%s' subscribed to current on '%s'",
        ...
        RCLCPP_INFO( 
            node_->get_logger(),
            "[PowerComponent] '%s' subscribed to voltage on '%s'",
            component_name_.c_str(),
            voltage_topic.c_str()
        );
    }
    ...
    // info for both
    PowerComponent::BatteryState PowerComponent::state() const{
        return BatteryState{
            state_current_.load(),
            state_voltage_.load()
        };
    ...
    void PowerComponent::set_active(bool on){
    ...
    bool PowerComponent::is_active() const{
        return active_.load();
    }
