    ...
    std::shared_ptr<liquidai_msgs::srv::RemoveEntity::Response> response)
{
    ...
    m_logger->debug("Request: [{}]", req.DebugString());
    ...
    if (executed) {
        if (result) {
            m_logger->debug("Response: [{}]", rep.data());
            response->result = rep.data();
        } else {
            m_logger->error("Service call failed");
            response->result = false;
        }
    } else {
        m_logger->error("Service call timed out");
        response->result = false;
    }
}
...
{
    int id = ecm_->EntityByComponents(
        gz::sim::components::Name(request->entity_name));
    response->id = id;
    m_logger->info("The id of {} is {}", request->entity_name, id);
}
...
GZ_ADD_PLUGIN(
    lotusim::gazebo::EntityManagement,
    gz::sim::System,
    lotusim::gazebo::EntityManagement::ISystemConfigure,
    lotusim::gazebo::EntityManagement::ISystemPreUpdate,
    lotusim::gazebo::EntityManagement::ISystemUpdate,
    lotusim::gazebo::EntityManagement::ISystemPostUpdate)