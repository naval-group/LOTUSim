...
import rclpy
from liquidai_msgs.srv import *
from gazebo_msgs.srv import ApplyLinkWrench
from liquidai_msgs.srv import SetThrusterState, SetThrusterEfficiency
...
class DisturbanceManager(Node):
    ...
    def __init__(self, name, **kwargs):
        ...
        self._disturbances = parse_nested_params_to_dict(
            self._disturbances, unpack_value=True)
        ...
        self._wrench_timer = self.create_timer(
            0.1, self._publish_wrench_disturbance)
            ...
            for key, d in self._disturbances.items():
                ...
                if t > d['starting_time'] and not d['is_applied']:
                    ...
                    ###########################################################
                        ...
                        self.set_thruster_state(
                            d['thruster_id'], bool(d['is_on']))
                    ###########################################################
                    elif d['type'] == 'propeller_efficiency':
                        self.set_propeller_efficiency(
                            d['thruster_id'], d['efficiency'])
                    ###########################################################
                    elif d['type'] == 'thrust_efficiency':
                        self.set_thrust_efficiency(
                            d['thruster_id'], d['efficiency'])
                    ...
                    if d['duration'] > 0:
                        if self.get_clock().now().nanoseconds > int((d['starting_time'] + d['duration']) * 1e9):
                            ...
                            ###########################################################
                                ...
                                self.set_thruster_state(
                                    d['thruster_id'], not bool(d['is_on']))
                            ###########################################################
                            elif d['type'] == 'propeller_efficiency':
                                self.set_propeller_efficiency(
                                    d['thruster_id'], 1.0)
    ...
    def _publish_wrench_disturbance(self):
        ...
        msg.wrench.force = Vector3(
            x=self._body_force[0], y=self._body_force[1], z=self._body_force[2])
        msg.wrench.torque = Vector3(
            x=self._body_torque[0], y=self._body_torque[1], z=self._body_torque[2])
    ...
    def set_current(self, velocity, horizontal_angle, vertical_angle):
        ...
        self.wait_for_service_completion(
            future,
            'Current velocity changed successfully at %f s! vel= %f m/s' % (
                time_in_float_sec(self.get_clock().now()), velocity),
    ...
    def set_link_wrench(self, force, torque, duration, starting_time):
        ...
        self._body_force = np.array(
            [float(self._body_force[i]) + force[i] for i in range(3)])
        self._body_torque = np.array(
            [float(self._body_torque[i]) + torque[i] for i in range(3)])
        ...
        req.start_time = rclpy.time.Time(
            seconds=secs, nanoseconds=nsecs).to_msg()
        req.duration = rclpy.time.Duration(
            seconds=d_secs, nanoseconds=d_nsecs).to_msg()

        future = self._service_cb['wrench'].call_async(req)
        self.wait_for_service_completion(
            future,
            'Link wrench perturbation applied!, body_name=%s, t=%.2f s' % (
                body_name, time_in_float_sec(self.get_clock().now())),
    ...
    def set_thruster_state(self, thruster_id, is_on):
        ...
        future = self._service_cb['thrusters']['state'][thruster_id].call_async(
            req)
        time = time_in_float_sec(self.get_clock().now())
        self.wait_for_service_completion(future,
                                         'Setting state of thruster #%d, state=%s, t=%.2f s' % (
                                             thruster_id, 'ON' if is_on else 'OFF', time),
    ...
    def set_propeller_efficiency(self, thruster_id, eff):
        ...
        future = self._service_cb['thrusters']['propeller_efficiency'][thruster_id].call_async(
            req)
        time = time_in_float_sec(self.get_clock().now())
        self.wait_for_service_completion(future,
                                         'Setting propeller efficiency of thruster #%d, eff=%s, t=%.2f s' % (
                                             thruster_id, eff, time),
    ...
    def set_thrust_efficiency(self, thruster_id, eff):
        ...
        future = self._service_cb['thrusters']['thrust_efficiency'][thruster_id].call_async(
            req)
        time = time_in_float_sec(self.get_clock().now())
        self.wait_for_service_completion(
            future,
            'Setting thrust efficiency of thruster #%d, eff=%s, t=%.2f s' % (
                thruster_id, eff, time),
...
def main():
    ...
    try:
        ...
        node = DisturbanceManager(
            'disturbance_manager', parameter_overrides=[sim_time_param])
        node.run()
