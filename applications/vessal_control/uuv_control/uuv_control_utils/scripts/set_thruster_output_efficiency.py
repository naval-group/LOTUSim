...
from liquidai_msgs.srv import SetThrusterEfficiency
...
def main():
    ...
    node.get_logger().info(
        'Set the thruster output efficiency for vehicle, namespace=' + node.get_namespace())
    ...
    node.get_logger().info('Setting thruster output efficiency #{} to {}%'.format(
        thruster_id, 100 * efficiency))

    vehicle_name = node.get_namespace().replace('/', '')

    srv_name = build_service_name(
        vehicle_name, thruster_id, 'set_thrust_force_efficiency')
