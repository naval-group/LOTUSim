...
import sys
import traceback
from liquidai_msgs.srv import SetFloat
...
def main():
    ...
    service_name = node.get_parameter(
        'service_name').get_parameter_value().string_value
    assert service_name in services, 'Possible service names are=' + \
        str(services)
    ...
    node.get_logger().info('Set scalar parameter, service=%s, data=%.2f' %
                           (service_name, data))
