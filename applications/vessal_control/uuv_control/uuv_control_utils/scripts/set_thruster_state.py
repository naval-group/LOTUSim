...
import rclpy
from liquidai_msgs.srv import SetThrusterState
...
def main():
    ...
    node.get_logger().info(
        'Set the state of thrusters for vehicle, namespace=' + node.get_namespace())
    ...
    if node.has_parameter('is_on'):
        is_on = bool(node.get_parameter(
            'is_on').get_parameter_value().bool_value)
    else:
    ...
    srv_name = build_service_name(
        vehicle_name, thruster_id, 'set_thruster_state')
    ...
    if not set_state.wait_for_service(timeout_sec=2):
        raise RuntimeError(
            'Service %s not available! Closing node...' % (srv_name))
    ...
    if duration > 0:
        ...
        try:
            ...
            if response.success:
                node.get_logger().info('Time={} s'.format(time_in_float_sec(node.get_clock().now())))
                node.get_logger().info('Returning to previous state of thruster #{}={}'.format(
