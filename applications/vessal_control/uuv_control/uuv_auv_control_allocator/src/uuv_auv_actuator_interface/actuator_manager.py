...
class ActuatorManager(Node):
    ...
    def __init__(self, name, **kwargs):
        ...
        self.fin_config = parse_nested_params_to_dict(
            self.fin_config, '.', True)
    ...
    def find_actuators(self):
        ...
        base = '%s' % (self.namespace, self.base_link)

        frame = '%s%d' % (
            self.namespace, self.thruster_config['frame_base'], 0)

        self.get_logger().info('Lookup: Thruster transform found %s -> %s' % (base, frame))
        trans = self.tf_buffer.lookup_transform(
            base, frame, rclpy.time.Time(), rclpy.time.Duration(seconds=1))
