...
from liquidai_msgs.msg import FloatStamped
...
class AUVGeometricTrackingController(Node):
    def __init__(self, name, world_ned_to_enu=None, **kwargs):
        ...
        self.thruster_topic = self.get_parameter_or_helper(
            'thruster_topic', 'thrusters/id_0/input').get_parameter_value().string_value
        ...
        self.init_thread = threading.Thread(
            target=self._init_async, daemon=True)
    ...
    def _init_async_impl(self):
        ...
        trans = self.tf_buffer.lookup_transform(
            base, frame, rclpy.time.Time(), rclpy.time.Duration(seconds=5))
        ...
        for i in range(self.n_fins):
            topic = self.build_fin_topic_name(
                self.fin_topic_prefix, i, self.fin_topic_suffix)
    ...
    def odometry_callback(self, msg):
        ...
        ref_msg.pose.position = geometry_msgs.Point(
            **self.to_dict_vect3(*des.p))
        ref_msg.pose.orientation = geometry_msgs.Quaternion(
            **self.to_dict_quat(*des.q))
        ref_msg.velocity.linear = geometry_msgs.Vector3(
            **self.to_dict_vect3(*des.vel[0:3]))
        ref_msg.velocity.angular = geometry_msgs.Vector3(
            **self.to_dict_vect3(*des.vel[3::]))
        ...
        error_msg.pose.position = geometry_msgs.Point(
            **self.to_dict_vect3(*e_p))
        ...
        # Limit yaw effort
        yaw_err = min(self.yaw_error_limit,
                      max(-self.yaw_error_limit, yaw_err))
        ...
        pitch_control = self.p_pitch * \
            self.unwrap_angle(
                pitch_des - rpy[1]) + self.d_pitch * (des.vel[4] - msg.twist.twist.angular.y)
        ...
        # Limit thrust
        thrust = min(self.max_thrust, self.p_gain_thrust *
                     np.linalg.norm(abs_pos_error) + self.d_gain_thrust * abs_vel_error)
