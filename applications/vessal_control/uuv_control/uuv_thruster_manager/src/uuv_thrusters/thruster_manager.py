...
class ThrusterManager(Node):
        ...
        if self.has_parameter('tam'):
            ...
            for i in range(self.n_thrusters):
                ...
                if list not in [type(params), type(conv_fcn)]:
                    ...
                    deduced_params = {
                        key: val.value for key, val in params.items()}
                    thruster = Thruster.create_thruster(
                        self, conv_fcn, i, topic, None, None,
                        **deduced_params)
                else:
                    # Unpack parameters to values
                    deduced_params = {key: val.value for key,
                                      val in params[i].items()}
                    thruster = Thruster.create_thruster(
    ...
    def update_tam(self, recalculate=False):
        ...
        base = self.config['base_link'].value
        # Strip /
        # base = base[1:]
        ...
        self.get_logger().info('conversion_fcn=' +
                               str(self.config['conversion_fcn'].value))
        self.get_logger().info('conversion_fcn_params=' +
                               str(self.config['conversion_fcn_params']))

        # for i in range(self.MAX_THRUSTERS):
        #     frame = self.config['thruster_frame_base'].value + str(i)
        ...
        #         self.get_logger().info("Looking up to transform: " + base + ' -> ' + frame)
        #         transformObject = self.tf_buffer.lookup_transform(base, frame,rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=30))
        ...
        # Thruster 0 left
        frame = self.config['thruster_frame_base'].value + str(0)
        pos = numpy.array([-2.374, 1.027, 0.318])
        quat = numpy.array([0.000, 0.000, 0.000, 1.000])
        topic = self.build_topic_name(self.config['thruster_topic_prefix'].value,
                                      0,
                                      self.config['thruster_topic_suffix'].value)
        # If not using robot_description, thrust_axis=None which will
        ...
        thruster = Thruster.create_thruster(
            self, self.config['conversion_fcn'].value,
            0, topic, pos, quat, thrust_axis, **params)
        self.thrusters.append(thruster)

        # Thruster 1 right
        frame = self.config['thruster_frame_base'].value + str(1)
        pos = numpy.array([-2.374, -1.027, 0.318])
        quat = numpy.array([0.000, 0.000, 0.000, 1.000])
        topic = self.build_topic_name(self.config['thruster_topic_prefix'].value,
                                      1,
        ...
        thruster = Thruster.create_thruster(
            self, self.config['conversion_fcn'].value,
            1, topic, pos, quat, thrust_axis, **params)
        self.thrusters.append(thruster)
    ...
    def compute_thruster_forces(self, gen_forces):
        ...
        if type(self.config['max_thrust'].value) == list:
            ...
            max_thrust = [
                self.config['max_thrust'].value for _ in range(self.n_thrusters)]
        for i in range(self.n_thrusters):
