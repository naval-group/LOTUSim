...
class TestThrusterAllocator(unittest.TestCase):
    ...
    def test_services_exist(self):
        ...
        services.append(self.node.create_client(
            ThrusterManagerInfo, '/{}/thruster_manager/get_thrusters_info'.format(
                NS)
        ))
        services.append(self.node.create_client(
            GetThrusterCurve, '/{}/thruster_manager/get_thruster_curve'.format(
                NS)
        ))
        services.append(self.node.create_client(
            SetThrusterManagerConfig, '/{}/thruster_manager/set_config'.format(
                NS)
        ))
        services.append(self.node.create_client(
            GetThrusterManagerConfig, '/{}/thruster_manager/get_config'.format(
                NS)
        ))

        for srv in services:
            self.assertTrue(srv.wait_for_service(timeout_sec=20),
                            'service %s not ready' % srv.srv_name)
    ...
    def test_config(self):
        ...
        self.assertEqual(len(tm_config.allocation_matrix),
                         tam_flat.size, tam_flat)
...
def generate_test_description():
    ...
    robot_state_description = launch_ros.actions.Node(
        ...
        parameters=[{'use_sim_time': False}, {
            'robot_description': doc}],  # Use subst here
    )
