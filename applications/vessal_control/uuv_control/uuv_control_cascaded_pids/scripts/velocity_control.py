...
class VelocityControllerNode(Node):
    def __init__(self, name, **kwargs):
        ...
        self._declare_and_fill_map(
            "linear_p", 1., "p component of pid for linear vel.", self.config)
        self._declare_and_fill_map(
            "linear_i", 0.0, "i component of pid for linear vel.", self.config)
        self._declare_and_fill_map(
            "linear_d", 0., "d component of pid for linear vel.", self.config)
        self._declare_and_fill_map(
            "linear_sat", 1., "saturation of pid for linear vel.", self.config)

        self._declare_and_fill_map(
            "angular_p", 1., "p component of pid for angular vel.", self.config)
        self._declare_and_fill_map(
            "angular_i", 0.0, "i component of pid for angular vel.", self.config)
        self._declare_and_fill_map(
            "angular_d", 0.0, "d component of pid for angular vel.", self.config)
        self._declare_and_fill_map(
            "angular_sat", 3.0, "saturation of pid for angular vel.", self.config)

        self._declare_and_fill_map(
            "vel_in_world", True, "Is current velocity supplied in world frame? (gazebo)", self.config)

        self._declare_and_fill_map(
            "current_vel_topic", "current_velocity", "topic name to get the vessel current speed.", self.config)
        self._declare_and_fill_map(
            "vel_cmd_topic", "cmd_vel", "topic name to get command velocity.", self.config)
        self._declare_and_fill_map(
            "accel_cmd_topic", "cmd_accel", "topic name to get command acceleration.", self.config)
        ...
        self.sub_cmd_vel = self.create_subscription(
            geometry_msgs.Twist, self.config['vel_cmd_topic'], self.cmd_vel_callback, 10)
        self.pub_cmd_accel = self.create_publisher(
            geometry_msgs.Accel, self.config['accel_cmd_topic'], 10)

    # ==============================================================================
    def cmd_vel_callback(self, msg):
        """Handle updated set velocity callback."""
        # Just store the desired velocity. The actual control runs on current_velocity callbacks
        v_l = msg.linear
        v_a = msg.angular
    ...
    def odometry_callback(self, msg):
        ...
        cmd_accel.linear = geometry_msgs.Vector3(
            x=a_linear[0], y=a_linear[1], z=a_linear[2])
        cmd_accel.angular = geometry_msgs.Vector3(
            x=a_angular[0], y=a_angular[1], z=a_angular[2])
...
def main():
    ...
    try:
        # TODO: review this hardcoding
        sim_time_param = sim_time_param = rclpy.parameter.Parameter(
            'use_sim_time',
        ...
        node = VelocityControllerNode(
            'velocity_control', parameter_overrides=[sim_time_param])
        rclpy.spin(node)
