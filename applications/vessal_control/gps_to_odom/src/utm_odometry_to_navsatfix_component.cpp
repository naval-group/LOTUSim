/*
 * Translates nav_msgs/Odometry in UTM coordinates back into
 * sensor_msgs/NavSat{Fix,Status} Useful for visualizing UTM data on a map or
 * comparing with raw GPS data Added by Dheera Venkatraman (dheera@dheera.net)
 */
...
public:
        ...
        : Node("utm_odometry_to_navsatfix_node", options)
    {
        ...
        auto callback =
            [this](const typename nav_msgs::msg::Odometry::SharedPtr odom)
            -> void {
            if (odom->header.stamp.sec == 0 &&
