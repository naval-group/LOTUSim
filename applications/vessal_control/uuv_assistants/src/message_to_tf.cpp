...
//   static inline void fromMsg(const geometry_msgs::msg::Point& msgIn,
//   tf2::Vector3& out)
//   {
    ...
    std::string child_frame_id = "")
{
    ...
    if (!g_footprint_frame_id.empty() &&
        child_frame_id != g_footprint_frame_id) {
    ...
    if (!g_footprint_frame_id.empty() &&
        child_frame_id != g_stabilized_frame_id) {
...
int main(int argc, char **argv)
{
    ...
    // Add node options to auto declare parameters from launch files and cmd
    // line
    ...
    if (!g_topic.empty()) {
        ...
        RCLCPP_FATAL(
            node->get_logger(),
            "Multicallbacks from ROS 1 not yet implemented");
    ...
    if (subscribers == 0) {
        ...
        RCLCPP_FATAL(
            ...
            "imu_topic and topic are set.\n"
            "Please specify exactly one of them or simply add the topic name "
            "to the command line.");
        return 1;
    }
